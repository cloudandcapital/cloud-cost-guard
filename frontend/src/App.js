...
import { Activity, AlertTriangle, Bot, Calendar, CheckCircle, Cloud, Download, Eye, Info, Layers, PieChart as PieIcon, ShieldCheck, X } from "lucide-react";
...
const UnavailableCard = ({ title, explanation }) => (
  <Card className="kpi-card shadow-sm" data-testid={`unavailable-${title.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}>
    <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-brand-muted">{title}</CardTitle></CardHeader>
    <CardContent className="pt-0">
      <div className="text-lg font-semibold text-brand-ink">{unavailable}</div>
      <p className="text-xs text-brand-muted mt-2">{explanation}</p>
    </CardContent>
  </Card>
);

const FindingCard = ({ finding, onDetails }) => (
  <Card className="finding-card shadow-sm" data-canonical-id={finding.id}>
    <CardHeader className="pb-3">
      <div className="flex items-start justify-between gap-3">
        <CardTitle className="text-base text-brand-ink">{finding.title}</CardTitle>
        <Badge className={`severity-${finding.severity}`}>{finding.severity.toUpperCase()}</Badge>
      </div>
      <CardDescription>{finding.type.replaceAll("_", " ")} · {finding.status}</CardDescription>
    </CardHeader>
    <CardContent className="space-y-3">
      <p className="text-sm text-brand-ink">{finding.context}</p>
      <div className="text-xs text-brand-muted">
        Producer: {finding.producer.name} {finding.producer.version} · Quality: {finding.quality}
      </div>
      <div className="text-xs text-brand-muted">Evidence: {finding.evidence_ids.join(", ")}</div>
      <Button variant="outline" size="sm" onClick={() => onDetails(finding)} className="w-full btn-brand-outline">
        <Eye className="h-3 w-3 mr-2" />View Details &amp; Methodology
      </Button>
    </CardContent>
  </Card>
);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-bg to-brand-light" data-testid="approved-dashboard" data-view-schema={model.schema}>
      <header className="nav-header" data-testid="dashboard-header">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="leading-tight"><h1 className="brand-title">Cloud+ Cost Guard</h1><p className="text-[15px] text-brand-muted">Canonical technology spend decision support <span className="text-xs opacity-60">· Illustrative</span></p></div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-4 text-xs text-brand-muted flex flex-wrap gap-3" data-testid="capability-disclosure">
        ...
        <section className="space-y-6 mb-8" data-testid="executive-summary">
          <Card className="kpi-card shadow-sm"><CardContent className="py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div><div className="text-xs font-semibold uppercase tracking-wider text-brand-muted">Published Technology Spend</div><div className="text-4xl font-bold text-brand-ink" data-canonical-id={model.total.id}>{money(model.total.displayValue, 4)}</div><div className="text-sm text-brand-muted mt-2">{periodLabel(model.identity.report_period)}</div></div>
            <div className="md:text-right"><div className="text-xs uppercase text-brand-muted">Trusted reconciliation</div><div className="text-2xl font-bold text-brand-success">Passed</div><div className="text-xs text-brand-muted">Difference {model.total.trace.currency} {model.reconciliation.difference}</div></div>
          </CardContent></Card>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6" data-testid="scope-cards">
            <MetricCard title="Cloud" value={money(model.cloud.total.displayValue)} subtitle="Canonical provider-billed scope, including native AI" icon={Cloud} />
            <MetricCard title="Direct AI" value={money(model.ai.directScope.displayValue, 4)} subtitle="Direct-vendor billing only; provider-billed AI remains in Cloud" icon={Bot} />
            <MetricCard title="SaaS" value={money(model.saas.scope.displayValue)} subtitle="Canonical same-period allocated scope" icon={Layers} />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <UnavailableCard title="Tagging coverage" explanation={unsupported("tagging_coverage")} />
            <UnavailableCard title="Projected next month" explanation={unsupported("next_month_forecast")} />
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <UnavailableCard title="Combined daily Technology Spend" explanation={unsupported("combined_daily_technology_spend")} />
            <Card className="kpi-card shadow-sm" data-testid="scope-donut-card"><CardHeader><CardTitle className="flex items-center gap-2"><PieIcon className="h-5 w-5" />Tech Spend by Scope</CardTitle><CardDescription>Three trusted inputs reconcile to the published total.</CardDescription></CardHeader><CardContent>
              <div className="h-56"><ResponsiveContainer><PieChart><Pie data={scopeChart} dataKey="value" innerRadius={55} outerRadius={88}>{scopeChart.map((entry) => <Cell key={entry.record.id} fill={entry.fill} />)}</Pie><Tooltip formatter={(value) => money(value, 4)} /></PieChart></ResponsiveContainer></div>
        ...
        <Tabs defaultValue="findings" className="space-y-6">
          ...
          <TabsContent value="findings" className="space-y-5"><div className="flex justify-between"><h2 className="font-brand-serif text-xl font-semibold">Canonical Findings</h2><Badge className="badge-brand">{model.findings.length} findings</Badge></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-6">{model.findings.map((item) => <FindingCard key={item.id} finding={item} onDetails={setFinding} />)}</div></TabsContent>

          <TabsContent value="products" className="space-y-5"><h2 className="font-brand-serif text-xl font-semibold">Cloud Services</h2><Card className="kpi-card"><CardContent className="pt-6"><Table><TableHeader><TableRow><TableHead>Canonical service</TableHead><TableHead>Provider</TableHead><TableHead className="text-right">Reporting-period cost</TableHead></TableRow></TableHeader><TableBody>{model.cloud.services.map((service) => <TableRow key={service.id}><TableCell>{service.dimensions.service}</TableCell><TableCell>{service.dimensions.provider.toUpperCase()}</TableCell><TableCell className="text-right">{money(service.displayValue)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card></TabsContent>
