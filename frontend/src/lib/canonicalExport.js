...
const formatCurrencyString = (value, digits = 2) => {
  if (typeof value !== "string" || !/^(0|[1-9]\d*)(\.\d+)?$/.test(value) || !Number.isInteger(digits) || digits < 0) throw new Error("Canonical export generation failed closed: invalid decimal formatting input");
  const [integer, fraction = ""] = value.split(".");
  const kept = fraction.padEnd(digits, "0").slice(0, digits);
  let scaled = `${integer}${kept}`;
  if (fraction.length > digits && fraction[digits] >= "5") {
    const characters = scaled.split("");
    let index = characters.length - 1;
    while (index >= 0 && characters[index] === "9") { characters[index] = "0"; index -= 1; }
    if (index < 0) characters.unshift("1"); else characters[index] = String(Number(characters[index]) + 1);
    scaled = characters.join("");
  }
  const integerLength = scaled.length - digits;
  const roundedInteger = scaled.slice(0, integerLength).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  const roundedFraction = digits ? `.${scaled.slice(integerLength).padStart(digits, "0")}` : "";
  return `$${roundedInteger}${roundedFraction}`;
};
const period = ({ start, end, timezone }) => `${start} through ${end} · half-open · ${timezone}`;
const slug = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");

const resolveExportModel = (candidate) => {
  const selected = getCcac11PresentationModel();
  if (candidate !== undefined && JSON.stringify(stable(candidate)) !== JSON.stringify(stable(selected))) {
    throw new Error("Canonical export generation failed closed: presentation model integrity mismatch");
  }
  return selected;
};
...
const assembleCanonicalEvidencePackage = (model) => {
  return stable({
    ...
    canonical_unsupported: Object.values(model.unsupported).map(canonical),
    presentation_unavailable: presentationUnavailable,
  });
};

export function buildCanonicalEvidencePackage(candidate) {
  return assembleCanonicalEvidencePackage(resolveExportModel(candidate));
}
...
export function buildCanonicalExecutiveHtml(model) {
  model = resolveExportModel(model);
  const evidence = assembleCanonicalEvidencePackage(model);
  ...
  const unavailableItems = (items) => items.map((item) => `<li><strong>${escapeHtml(item.concept.replaceAll("_", " "))}</strong><span>${escapeHtml(item.explanation)}</span><code>${escapeHtml(item.reason_code)}</code></li>`).join("");
  const invoiceCards = model.saas.invoices.map((invoice) => `<article><p class="kicker">${escapeHtml(invoice.dimensions.billing_cadence)} invoice · separate period</p><h3>${escapeHtml(invoice.dimensions.application)}</h3><p class="metric">${formatCurrencyString(invoice.value)}</p><p>Exact canonical value: USD ${escapeHtml(invoice.value)}</p><p>${escapeHtml(period(invoice.trace.period))}. This invoice is not added to the canonical SaaS scope or to the other invoice.</p>${traceHtml(invoice)}</article>`).join("");
  return `<!doctype html>
...
<section><p class="eyebrow">Published canonical total</p><div class="hero"><div class="card wide"><h2>Technology Spend</h2><p class="metric">${formatCurrencyString(model.total.value)}</p><p class="exact">Exact canonical value: USD ${escapeHtml(model.total.value)}</p><p>Reconciliation ${escapeHtml(model.reconciliation.status)} · exact difference USD ${escapeHtml(model.reconciliation.difference)}</p>${traceHtml(model.total)}</div><div class="card"><h3>Cloud</h3><p class="metric">${formatCurrencyString(model.cloud.total.value)}</p><p class="exact">Exact USD ${escapeHtml(model.cloud.total.value)}</p><p>Additive; provider-billed native AI included.</p></div><div class="card"><h3>Direct AI</h3><p class="metric">${formatCurrencyString(model.ai.directScope.value,4)}</p><p class="exact">Exact USD ${escapeHtml(model.ai.directScope.value)}</p><p>Additive; direct-vendor billing only.</p></div><div class="card"><h3>SaaS</h3><p class="metric">${formatCurrencyString(model.saas.scope.value)}</p><p class="exact">Exact USD ${escapeHtml(model.saas.scope.value)}</p><p>Additive; same-period allocated scope.</p></div></div></section>
<section><h2>Broader AI analysis</h2><div class="card"><p class="metric">${formatCurrencyString(model.ai.domainTotal.value,4)}</p><p class="exact">Exact canonical value: USD ${escapeHtml(model.ai.domainTotal.value)}</p><p><strong>Non-additive.</strong> Includes provider-billed AI already represented in Cloud and must not be added to Technology Spend.</p>${traceHtml(model.ai.domainTotal)}</div></section>
...
<section><h2>Unavailable registry</h2><p>Unsupported measures remain explicit and visually secondary; missing values are never substituted or invented.</p><h3>Canonical unsupported</h3><p>Declarations taken directly from the validated CCAC 1.1 presentation model.</p><ul class="unavailable">${unavailableItems(evidence.canonical_unsupported)}</ul><h3>Presentation unavailable</h3><p>Dashboard and export coverage declarations derived from validated absence boundaries.</p><ul class="unavailable">${unavailableItems(evidence.presentation_unavailable)}</ul></section>
...
export function buildCanonicalExportFiles(candidate) {
  const model = resolveExportModel(candidate);
...
export { EXPORT_SCHEMA, HTML_MIME, JSON_MIME, escapeHtml, formatCurrencyString };
