...
export function createCcac11PresentationModel(view = generatedView) {
  ...
  return { schema: view.schema, identity, sourceMetadata: metadata, total, reconciliation, scopes, scopeById, cloud: { total: scopes[0], daily: cloudDaily, services: cloudServices, comparison: cloudComparison }, ai: { directScope: scopes[1], domainTotal: aiDomainTotal, costMetrics: aiCostMetrics, crossDomainAdditivity: view.ai.cross_domain_additivity }, saas: { scope: scopes[2], invoices }, findings, anomalies, resilience: { classification: view.resilience.recoverability_classification, findings: resilienceFindingRaw, modeled: modeledResilience, observed: observedResilience }, opportunity: view.opportunity, unsupported };
}

export function getCcac11PresentationModel() { return createCcac11PresentationModel(generatedView); }
