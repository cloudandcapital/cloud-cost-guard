...
`ccg-dashboard-view/1.1.0` now consumes the already validated CCAC 1.1 illustrative `trusted_report` in `fixtures/ccac/illustrative-v0.3.0/run`. It is not a replacement for CCAC validation. The isolated `ccac-dashboard-view-v1.1.generated.json` artifact is not imported into React, does not change public numbers or exports, and is not provided to Lumen.
...
| `source_metadata.manifest_sha256/artifact_sha256s` | `provenance` | Command Center 0.3.0 | SHA-256 strings | validated provenance | Sorted keys; missing fails | Connects records to immutable source artifacts |
...
| `technology_spend.total/scopes/reconciliation` | `metric.tech-spend.total`, the three `metric.tech-spend.scope.*` inputs, and `reconciliation.tech-spend` | Command Center 0.3.0 and canonical scope owners | decimal strings / USD / 2026-07-01..22 | calculated or observed, valid, additive | Exact copy; reconciliation difference and tolerance become decimal strings | Makes CCAC 1.1 spend available only to the isolated data artifact |
