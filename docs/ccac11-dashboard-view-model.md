...
The React cutover changes only the visible dashboard's data source and truthful presentation. The selector validates the schema, illustrative/complete identity, pinned report and release provenance, reconciliation, canonical identities, decimal values, and unsupported registry before returning any model. A failure renders one application-level error and never falls back to legacy financial data. Browser conversion to numbers is limited to display and chart geometry; the published total is never recomputed.

Export migration, unified CCAC 1.1 Lumen grounding, `/api/ask-claude`, real ingestion, responsive redesign, and later roadmap phases remain out of scope. Legacy artifacts can remain for compatibility, but `App.js` does not import them or combine their values with this view.
