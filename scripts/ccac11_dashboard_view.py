...
VIEW_SCHEMA = "ccg-dashboard-view/1.1.0"
POLICY_PATH = Path(__file__).with_name("ccac11_dashboard_view_policy_v1.json")
...
def _validate_technology_spend_reconciliation(
    report: dict[str, Any], metrics: dict[str, dict[str, Any]]
) -> dict[str, Any]:
    """Independently verify CCAC 1.1's published total without replacing it."""
    scope_ids = [
        "metric.tech-spend.scope.cloud",
        "metric.tech-spend.scope.direct_ai",
        "metric.tech-spend.scope.saas",
    ]
    total_id = "metric.tech-spend.total"
    owners = {
        scope_ids[0]: "finops-lite",
        scope_ids[1]: "ai-cost-lens",
        scope_ids[2]: "saas-cost-analyzer",
    }
    report_period = report.get("period")
    scope_values: list[Decimal] = []
    for metric_id in scope_ids:
        metric = metrics.get(metric_id)
        if not isinstance(metric, dict):
            _fail("canonical Technology Spend scope is missing")
        value = _exact_decimal(metric.get("value"), f"{metric_id}.value")
        if value < 0:
            _fail("canonical Technology Spend scope cannot be negative")
        ...
        if (
            ...
            or metric.get("period") != report_period
            or not isinstance(boundary, dict)
            or boundary.get("relationship") != "canonical_scope_spend"
            or boundary.get("canonical_owner") != owners[metric_id]
            or boundary.get("coverage") != "complete"
            or boundary.get("total_eligible") is not True
        ):
            _fail("canonical Technology Spend scope is ineligible")
        scope_values.append(value)

    total = metrics.get(total_id)
    if not isinstance(total, dict):
        _fail("published Technology Spend total is missing")
    published_total = _exact_decimal(total.get("value"), f"{total_id}.value")
    if published_total < 0:
        _fail("published Technology Spend total cannot be negative")
    if (
        not isinstance(total.get("input_metric_ids"), list)
        or len(total["input_metric_ids"]) != len(scope_ids)
        or set(total["input_metric_ids"]) != set(scope_ids)
        or total.get("period") != report_period
        or total.get("currency") != "USD"
        or total.get("unit") != "currency"
        or total.get("basis") != "calculated"
        or total.get("quality_status") != "valid"
        or total.get("additivity") != "additive"
        or total.get("metric_role") != "technology_spend_total"
    ):
        _fail("published Technology Spend total is incompatible")

    independently_calculated = sum(scope_values, Decimal("0"))
    if published_total != independently_calculated:
        _fail("published Technology Spend total does not reconcile")

    reconciliations = report.get("reconciliation")
    if not isinstance(reconciliations, list) or len(reconciliations) != 1:
        _fail("Technology Spend reconciliation is incomplete")
    reconciliation = reconciliations[0]
    if (
        not isinstance(reconciliation, dict)
        or reconciliation.get("id") != "reconciliation.tech-spend"
        or reconciliation.get("reconciliation_type") != "technology_spend_total"
        or not isinstance(reconciliation.get("input_metric_ids"), list)
        or len(reconciliation["input_metric_ids"]) != len(scope_ids)
        or set(reconciliation["input_metric_ids"]) != set(scope_ids)
        or reconciliation.get("output_metric_id") != total_id
        or reconciliation.get("status") != "passed"
    ):
        _fail("Technology Spend reconciliation is incompatible")
    difference = _exact_decimal(
        reconciliation.get("difference"), "reconciliation.difference"
    )
    tolerance = _exact_decimal(
        reconciliation.get("tolerance"), "reconciliation.tolerance"
    )
    actual_difference = published_total - independently_calculated
    if tolerance < 0 or difference != actual_difference or abs(difference) > tolerance:
        _fail("Technology Spend reconciliation difference is invalid")
...
def _validate_projection_policy(
    ...
    """Enforce the fixed, human-reviewed ccg-dashboard-view/1.1.0 policy."""
    policy = PROJECTION_POLICY
    if (
        policy.get("policy_schema") != "ccg-dashboard-projection-policy/1.1.0"
        or policy.get("view_schema") != VIEW_SCHEMA
        or policy.get("approved_release") != APPROVED_RELEASE
    ):
    ...
    expected_metric_ids = set().union(*expected_metrics_by_producer.values()) | set(
        policy["display"]["headline_metric_ids"]
    )
    ...
    for metric_id, metric in metrics.items():
        metric_owner_by_id[metric_id] = (
            "technology-spend"
            if metric_id == "metric.tech-spend.total"
            else next(
                producer
                for producer, ids in expected_metrics_by_producer.items()
                if metric_id in ids and producer != "technology-spend"
            )
        )
    ...
    for evidence_id, metric_ids in actual_metric_evidence.items():
        producer_metric_ids = [
            ...
            if metric_owner_by_id[metric_id] != "technology-spend"
        ]
        if not producer_metric_ids:
            continue
        producers = {metric_owner_by_id[metric_id] for metric_id in producer_metric_ids}
        if len(producers) != 1:
    ...
    # The complete evidence graph is pinned above. CCAC 1.1's reconciled total
    # legitimately references evidence owned by multiple scope producers, so a
    # second single-owner partition is no longer a valid invariant.
    ...
    if not isinstance(provenance, dict) or provenance != {
        "manifest_sha256": policy["manifest"].get(
            "report_provenance_sha256", policy["manifest"]["sha256"]
        ),
...
def _metric_owner(metric_id: str) -> str:
    if metric_id == "metric.tech-spend.total":
        return "technology-spend"
    scope_owners = {
        "metric.tech-spend.scope.cloud": "finops-lite",
        "metric.tech-spend.scope.direct_ai": "ai-cost-lens",
        "metric.tech-spend.scope.saas": "saas-cost-analyzer",
    }
    if metric_id in scope_owners:
        return scope_owners[metric_id]
    prefixes = {
...
def _trace(producer: str, source_id: str, record: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "producer": (
            {"name": "tech-spend-command-center", "version": "0.3.0"}
            if producer == "technology-spend"
            else {"name": producer, "version": EXPECTED_PRODUCERS[producer]}
        ),
        "source_artifact": (
            "report.json"
            if producer == "technology-spend"
            else ARTIFACT_NAMES[producer]
        ),
        "basis": (
...
def _metric_record(
    ...
    actual_policy = {
        "basis": trace["basis"],
        "quality_status": trace["quality"],
        "additivity": trace["additivity"],
        "currency": trace["currency"],
        "unit": trace["unit"],
        "period": trace["period"],
    }
    if actual_policy != PROJECTION_POLICY["metric_semantics"].get(metric_id):
        _fail(
...
def project_dashboard_view(report: Any) -> dict[str, Any]:
    ...
    technology_spend_reconciliation = _validate_technology_spend_reconciliation(
        report, metrics
    )
    ...
    if (
        _id_list(
            display.get("opportunity_aggregate_ids"),
            "display.opportunity_aggregate_ids",
        )
        != []
    ):
    ...
    ) | {"metric.tech-spend.total"} != set(metrics):
    ...
    cloud_total_id = "metric.tech-spend.scope.cloud"
    required_cloud = {
        cloud_total_id,
    ...
    if set(opportunities) != {opportunity_id} or aggregates:
    ...
    if overlap.get("disposition") != "none_known":
    ...
    return {
        ...
        "technology_spend": {
            "total": metric_records["metric.tech-spend.total"],
            "scopes": [metric_records[item] for item in sections["technology-spend"]],
            "reconciliation": {
                **technology_spend_reconciliation,
                "difference": _decimal(
                    technology_spend_reconciliation.get("difference"),
                    "reconciliation.difference",
                ),
                "tolerance": _decimal(
                    technology_spend_reconciliation.get("tolerance"),
